...
metrics <- function(Org_id, curr_work_dir) {
  ...
  #metrics_dir <- Directories_org.df$MetricsDir
...
headers <- list("Sample", "Num_Reads", "Coverage", "MeanContigLength", "N50ContigLength", "Comments")

names(MetricsUpload.df) <- headers
#View(MetricsUpload.df)
...
#Metrics_all.df <- as_tibble(read.csv(paste(local_output_dir, "output_METRICS.csv", sep = ""),  header = TRUE, sep = ",", stringsAsFactors = FALSE)) #this line only for debugging
...
#shell.exec("C:\\WGS_Typing\\Output\\LabWareUpload_METRICS.csv")
