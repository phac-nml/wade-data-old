...
labware_gas_amr <- function(Org_id, curr_work_dir) {
  ...
  #cat("\n directory string passed to function: ", curr_work_dir, "\n", "for Org_id =", Org_id, "\n")
...
for (m in 1L:NumSamples)  #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
{
  ...
  if (lw_pbp2x != "WT")
  {
    ...
    if (lw_pbp2x_550 != "WT")
    {
      ...
      {lw_pbp2x_prof <- paste("pbp2x ", lw_pbp2x_550, sep = "")} else
      {lw_pbp2x_prof <- paste(lw_pbp2x_prof, "/", lw_pbp2x_550, sep = "")}
    }
...
#write.csv(lw_output_good.df, paste(local_output_dir, "LabWareUpload_GAS_AMR_good.csv", sep = ""), quote = FALSE,  row.names = FALSE)
...
#cat("\nDone! Returning output to shiny main page...\n")
