...
#new_folder <- askYesNo("Get new folder location?")
...
if (Org_id == "GONO")
{
  #vcf_folder <- "W:\\Projects\\Project_GC_WalterD\\MiSeq\\Gonorrhoea\\vcf"
  rRNA23S_position1 <- "23S4_NCCP11945	2045"
  rRNA23S_position2 <- "23S4_NCCP11945	2597"
  #header <- list("SampleNo", "A2045G", "C2597T")

}
if (Org_id == "PNEUMO")
{
  #vcf_folder <- "W:\\Projects\\Project_GC_WalterD\\MiSeq\\Streptococcus\\Pneumo\\vcf"
  rRNA23S_position1 <- "23S_rRNA_R6_sprr02	2061"
  rRNA23S_position2 <- "23S_rRNA_R6_sprr02	2613"
  #header <- list("SampleNo", "A2061G", "C2613T")
}
...
if(SampleNo == "list")
{
  #SampleList.df <- read.csv("list.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
...
for (m in 1L:NumSamples)  #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
{
  ...
  {
    #stop("VCF file for that sample number not found!")
  ...
  for (i in 1:length(linn))
  {
      ...
      if (str_detect(linn[i], rRNA23S_position1))  #2597 & 2045 for GONO; 2061 for pneumo
      {
        ...
        #DP <- as.integer(vcf_parts[9])   #total number of reads (depth of reads)
        AO1 <- vcf_parts[6]
        #AO <- as.integer(vcf_parts[13])  #alternate observations from reference
...
#View(OutputProfile.df)
outfile <- paste(local_output_dir, "output_profile_23S.csv", sep = "")
#write.csv(OutputProfile.df, "C:\\WGS_Typing\\Output\\output_profile_23S.csv", row.names = F)
