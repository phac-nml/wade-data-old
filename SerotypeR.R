...
SEROTYPE_pipeline <- function(Org_id, SampleNo, LocusID, Test_id, curr_work_dir){
  ...
  #cat("Org_id: ", Org_id, " Test_id: ", Test_id, " SampleNo: ", SampleNo, " LocusID: ", LocusID, sep = "")
  # curr_work_dir <- "L:\\GC_PAHO\\Whole_Genome_Sequencing\\WGS_Typing\\"
  # Org_id <- "PNEUMO"               #GAS, PNEUMO or GONO
  # Test_id <- "SERO"
  # SampleNo <- "SC20-4212-P"     #"L:\\GC_PAHO\\Whole Genome Sequencing\\WGS Typing\\list.csv"
  # LocusID <- "list"             #L:\\GC_PAHO\\Whole Genome Sequencing\\WGS Typing\\GAS\\Wamr_R\\temp\\loci.csv"
  #LocusID <- "25_wcyD"
  ...
  #Main_Dir <- paste(system_dir, Org_id, " MolecularSerotyping\\", sep = "")
  ...
  if (Test_id == "SERO")  #use CPS blastn
  {
  CPS_types <- c("01", "02", "03", "04", "05", "08", "13", "14", "20", "21",
                 "24", "27", "29", "31", "34", "36", "37", "38", "39", "40",
                 "42", "43", "44", "45", "46", "47A", "47F", "48",
                 "16F", "16A", "17A", "17F", "19A", "19F", "10D",
                 #"23A", "23B", "23B1", "23F",
                 #"24A", "24B", "24F",
                 "32A", "32F",
                 "Swiss_NT",
  ...
  if (Test_id == "SERO2")  #use SNP based approach for GBS only
  {
    CPS_types <- c("01", "02", "03", "04", "05", "08", "13", "14", "20", "21",
                   "24", "27", "29", "31", "34", "36", "37", "38", "39", "40",
                   "42", "43", "44", "45", "46", "47A", "47F", "48",
  ...
  #------------------------------------------------------------------------------------------------------
  #prepare allele lookup for BLASTing
  #LocusLkupDNA <- paste(Lkup_Dir, "12_wciI.fasta", sep="")
  #BlastFormatCommand <- paste("makeblastdb -in ", LocusLkupDNA, " -dbtype nucl", sep = "")
  #try(system(BlastFormatCommand))
  #------------------------------------------------------------------------------------------------------
  #SampleOutputProfile.df <- tibble(SampleNo = NA, Serotype = NA, Profile = NA)
  outfile_nf <- paste(local_output_dir, "output_dna_notfound.fasta", sep = "")
  unlink(outfile_nf) #this deletes the file!
  #unlink("C:\\WGS_Typing\\Output\\output_profile_PNEUMO_SEROTYPE.csv")

  #outfile <- paste("C:\\WGS_Typing\\Output\\LabWareUpload_", Org_id, "_SEROTYPE.csv", sep = "")
  ...
  for (m in 1L:NumSamples)  #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  {
    ...
    #file.copy(ContigFile, ContigFileLocal, overwrite = T)
    ...
    {SampleFound <- FALSE
     ...
     SampleProfile <- "Sample_Err"
      ...
      }else
      {
        ...
        #df.blastout <- arrange(df.blastout, desc(Ident))
        serotype1 <- df.blastout$Allele[1]

        SerotypeParts <- unlist(strsplit(serotype1, "_"))
        serotype <- SerotypeParts[1]

        if (Org_id == "PNEUMO")
        {
          serogroup <- as.character(substr(serotype, 1, 2))
        }
        if (Org_id == "GBS")
        {
          serogroup <- "GBS"
    ...
    cat("Serogroup ", serogroup, " (", serotype1, ")!\n")
    ...
    for(p in 1L:NumLoci)
    {
      ...
      if(!file.exists(LocusFile))
      {
        ...
        #BlastCommand <- paste("blastn -query ", LocusFile, " -db ", ContigFileLocal, " -out C:\\WGS_Typing\\temp\\blastout.txt -evalue ", Blast_evalue, sep = "")
        ...
        #OutputLocus.df[1, col2_name] <- BlastResult  #result
        OutputLocus.df[col2_name] <- BlastResult  #result

        if (locus_result_type == "result")
        {
          #OutputLocusProfile.df[1, col2_name] <- BlastResult  #result
          OutputLocusProfile.df[col2_name] <- BlastResult
          SampleProfile <- paste(SampleProfile, ProfileSeparator, locus, "[", BlastResult, "]", sep="")
        ...
        if (BlastResult == "POS")
        {
          ...
          #update locus ouput
          #OutputLocus.df[1, col5_name] <- nonsence_mutation  #pseudo
          OutputLocus.df[col5_name] <- nonsence_mutation  #pseudo
          if (locus_result_type == "pseudo")
          {
            #OutputLocusProfile.df[1, col5_name] <- nonsence_mutation  #pseudo
            OutputLocusProfile.df[col5_name] <- nonsence_mutation
            SampleProfile <- paste(SampleProfile, ProfileSeparator, locus, "[", nonsence_mutation, "]", sep="")
          ...
          #update locus ouput
          #OutputLocus.df[1, col4_name] <- motifs  #mutations
          OutputLocus.df[col4_name] <- motifs  #mutations
          if (locus_result_type == "mutations")
          {
            #OutputLocusProfile.df[1, col4_name] <- motifs  #mutations
          ...
          if(LocusLkupDNApresent)
          {
            ...
            #update locus ouput
            #OutputLocus.df[1, col3_name] <- Allele  #allele
            OutputLocus.df[col3_name] <- Allele
            if (locus_result_type == "allele")
            {
              #OutputLocusProfile.df[1, col3_name] <- Allele  #allele
              OutputLocusProfile.df[col3_name] <- Allele  #allele
              SampleProfile <- paste(SampleProfile, ProfileSeparator, locus, "[", Allele, "]", sep="")
    ...
    #SampleOutputProfile.df$SampleNo[m] <- CurrSampleNo
    #SampleOutputProfile.df$Serotype[m] <- serotype
    #SampleOutputProfile.df$Profile[m] <- SampleProfile
  ...
  if (LocusID == "list")
  {
  #outfile <- "C:\\WGS_Typing\\Output\\LABWARE_PNEUMO_SEROTYPER.csv"
  write.csv(SampleOutputProfile.df, outfile, quote = FALSE,  row.names = F)
  }

  if (NumSamples == 1L)
  {
    #outfile2 <- "C:\\WGS_Typing\\Output\\output_profile_PNEUMO_SEROTYPE.csv"
    #write.csv(OutputLocus.df, outfile, quote = FALSE, row.names = F )
